...
do
    ...
    cp "${NODE_DIR}/node-v${nodejs_version}-${os}/bin/node" ".sea/sensibo-mqtt-${os}"
    rm -rf "${NODE_DIR}/node-v${nodejs_version}-${os}"
    npx postject ".sea/sensibo-mqtt-${os}" NODE_SEA_BLOB .sea/prep.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2
done
